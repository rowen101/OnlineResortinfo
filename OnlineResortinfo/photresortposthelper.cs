...
{
    public class photresortposthelper
    {
        public static Image GetThumbnail(int photoid)
        {
            string sql = "SELECT IMAGE,THUMBNAILHEIGHT,THUMBNAILWIDTH FROM tbl_post WHERE PST_ID=@id";
        ...
        {
            string sql = "SELECT IMAGE FROM tbl_post WHERE PST_ID=@id";
            SqlParameter[] p = new SqlParameter[1];
            p[0] = new SqlParameter("@id", photoid);
            SqlDataReader reader = SqlHelper.ExecuteReader(sql, p);
            byte[] imagedata = null;

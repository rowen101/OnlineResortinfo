...
{
    public class userviewerphotohelper
    {
        public static Image GetThumbnail(int photoid)
        {
            string sql = "SELECT IMAGE,THUMBNAILHEIGHT,THUMBNAILWIDTH FROM tbl_user WHERE ID=@id";
        ...
        public static Image GetThumbnailForAlbum(int albumid)
        {
            ...
            int photoid = int.Parse(SqlHelper.ExecuteScalar("SELECT TOP 1 ID FROM tbl_user WHERE ID=@albumid ORDER BY DATE DESC", p).ToString());
            return GetThumbnail(photoid);
        }

        public static Image GetPhoto(int photoid)
        {
            string sql = "SELECT IMAGE FROM tbl_user WHERE ID=@id";
            SqlParameter[] p = new SqlParameter[1];
            p[0] = new SqlParameter("@id", photoid);
            SqlDataReader reader = SqlHelper.ExecuteReader(sql, p);
            byte[] imagedata = null;
